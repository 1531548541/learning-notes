...
public class ContextLoader {
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
    	...
    	*/
		...
		try {
			...
			// 1. 如果当前成员变量中不存在WebApplicationContext则创建一个根WebApplicationContext
			if (this.context == null) {
				this.context = createWebApplicationContext(servletContext);
			}

			//XmlWebApplicationContext(xml驱动)和AnnotationConfigWebApplicationContext(注解驱动)都是复合的，都会进来
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// 2.配置并刷新整个根IoC容器，在这里会进行Bean的创建和初始化
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		 ...
		 * 		<param-value>zimu-2021-11-11</param-value>
		 * 	</context-param>
		 */
		...
		// 4.自定义配置上下文环境
		//检查web.xml是否有一些其余初始化类的配置，极大多数情况都不需要
	...
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		 ...
		 * 		<param-value>com.itheima.init.MyContextInitializerClasses</param-value>
		 * 	</context-param>
		 *
