...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			// 这个方法会注册3个默认环境 bean：environment、systemProperties 和 systemEnvironment，注册 2个bean 后置处理器
			...
			try {
				...
				//逻辑：找出所有实现BeanPostProcessor接口的类,分类、排序(按实现PriorityOrdered接口、Ordered接口、啥也没实现，三种情况排序)、注册
				...
				initApplicationEventMulticaster(); // ===> 就是往factory加了个single bean

				// Initialize other special beans in specific context subclasses.
				// 9、【刷新:拓展方法】这是个protected空方法，交给具体的子类来实现
				//  可以在这里初始化一些特殊的 Bean，例：springBoot在onRefresh() 完成内置Tomcat的创建及启动
				onRefresh();
	...
	protected void prepareRefresh() {
		...
		// 1.将 active 属性设置为 true，closed 属性设置为 false，它们都是 AtomicBoolean 类型
		...
		// 2. protected，交给子类实现：在上下文环境中初始化属性源
		initPropertySources();
